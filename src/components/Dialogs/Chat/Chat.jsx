...
const Chat = (props) => {
  ...
  const LoadMessages = () => {
    ...
    const elementsArr = [],
      resultArr = [];
    props.messages
      .filter((el) => el.dialogId === props.activeDialog)
      .forEach((messageObj, i, self) => {
        if (messageObj === self.at(-1)) lastMessageId = i;
        if (self.length !== 0 && i !== 0 && currentDate == messageObj.date)
          elementsArr[level].push(
            <Message
              index={i}
          ...
          elementsArr.push([]);
          elementsArr[++level].push(
            <span id={props.activeDialog} className={s.dateSpan}>
    ...
    props.activeDialog && console.log(elementsArr[0][0].props.className);
    // if (props.activeDialog) debugger;
  ...
  useEffect(() => {
    LoadMessages();
    loaded = true;
    if (lastMessageId !== oldMessageId) {
      setMessageElements(LoadMessages());
      loaded = false;
      oldMessageId = lastMessageId;
    }
    if (messagesElements.length && loaded && lastMessageId == oldMessageId)
      scrollToBottom();
  });
      ...
      <MessagesHeader
        ...
        name={
          props.activeDialog !== 0
            ? props.dialogs[props.activeDialog - 1].name
            : ""
        }
      />
